...
def make_total_token(data):
    data["total_token"] = data["total"].progress_apply(lambda x: tok._tokenize(x))
    return data


def make_summary_token(data):
    data["summary_token"] = data["summary"].progress_apply(lambda x: tok._tokenize(x))
    return data
...
if __name__ =="__main__":
    ...
    for file in tqdm(file_list):
        with open(os.path.join(file_dir, file), encoding="utf-8") as f:
            data = json.load(f)
            pre_data = pd.concat([pre_data, pd.DataFrame(data)], ignore_index=True)
    ...
    # pre_data["summary_token"] = pre_data.summary.progress_apply(lambda x: tok._tokenize(x))
